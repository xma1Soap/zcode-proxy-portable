...
import consoleHtml from "./console.txt" with { type: "text" };
import managerHtml from "./manager.txt" with { type: "text" };
...
import { getRuntimeSnapshot } from "../proxy/runtime-status.js";
import type { ResponseStore } from "../responses/store.js";
import { WebLoginService } from "./routes-auth.js";
import { handleManagerApi } from "./manager.js";
...
interface ServerOptions {
  ...
  /** Injected WebUI login service (tests). */
...
export function createFetchHandler(opts: ServerOptions): (req: Request) => Promise<Response> {
  ...
  const webLogin = opts.webLogin ?? new WebLoginService({ config, auth, fetchImpl: opts.fetchImpl });
  ...
  return async (req: Request): Promise<Response> => {
    ...
    if (method === "GET" && (path === "/" || path === "/manager")) {
      return new Response(renderWebui(managerHtml, config.auth.proxyApiKey, config.server.port), {
        status: 200,
        headers: {
    ...
    if (method === "GET" && (path === "/console" || path === "/embed" || path === "/webui" || path.startsWith("/webui/"))) {
      const page = path === "/webui" || path.startsWith("/webui/") ? webuiHtml : consoleHtml;
      return new Response(renderWebui(page, config.auth.proxyApiKey, config.server.port), {
        status: 200,
        headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-cache" },
      });
    }
    if (method === "GET" && path === "/manager/api/list") {
      const listed = await handleManagerApi(req, config);
    ...
    // Provider OAuth redirect has no proxy key — must stay public.
    if (WebLoginService.isPublicPath(path, method)) {
      const publicAuth = await webLogin.handle(req);
    ...
    const authRoute = await webLogin.handle(req);
    if (authRoute) return authRoute;

    const managerRoute = await handleManagerApi(req, config, process.cwd(), {
      applyCredential: async (cred) => {
        await webLogin.applyPortable(cred);
      },
      exportCredential: async () => {
        const live = auth.peekOAuthCredential();
        if (live) return live;
        const fromDisk = await loadCredential();
        if (!fromDisk) throw new Error("no credential on this port to export");
        return fromDisk;
      },
    });
    ...
    if (path === "/v1/models" && method === "GET") {
      return handleListModels(config);
    }
...
/** Fill the WebUI's default proxy key from config so chat works without a settings step. */
export function renderWebui(html: string, proxyApiKey?: string, port: number = 0): string {
  return html
    .replaceAll("__ZCODE_PROXY_KEY_JSON__", JSON.stringify(proxyApiKey ?? ""))
    .replaceAll("__ZCODE_PROXY_PORT_JSON__", JSON.stringify(port));
}
