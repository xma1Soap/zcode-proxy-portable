...
describe("AuthManager", () => {
  ...
  it("clearOAuthCredential drops live oauth state", async () => {
    const mgr = new AuthManager({ mode: "oauth", provider: "bigmodel" });
    mgr.setOAuthCredential({ apiKey: "oa", provider: "bigmodel", jwt: "jwt" });
    expect(mgr.hasLiveCredential()).toBe(true);
    mgr.clearOAuthCredential();
    expect(mgr.hasLiveCredential()).toBe(false);
    expect(mgr.peekOAuthCredential()).toBeNull();
    expect(mgr.getCredential()).rejects.toThrow(/not available/);
  });
});
