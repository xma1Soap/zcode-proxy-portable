...
export abstract class AuthCodeOAuthClient {
  ...
  /**
   * Build an authorize URL that redirects back to an existing HTTP listener
   * (the proxy's `/auth/callback`). Used by the WebUI so login does not need
   * a second localhost port — the user's browser returns to the same origin
   * they opened `/webui` on.
   */
  prepare(callbackUrl: string): { authorizeUrl: string; state: string } {
    const state = randomBytes(32).toString("hex");
    return { authorizeUrl: this.buildAuthorizeUrl(callbackUrl, state), state };
  }
